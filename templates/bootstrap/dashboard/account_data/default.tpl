  <div class="col-lg-4">
    ...
    {if $GLOBAL.userdata.donate_percent > 0}
            you donate <font color="green">{$GLOBAL.userdata.donate_percent|escape}%</font>./このプールは料金を払わず、寄付を<font color="green">{$GLOBAL.userdata.donate_percent|escape}%</font>しています。
    {else}
            you are not <a href="{$smarty.server.SCRIPT_NAME}?page=account&action=edit">donating</a>./このプールは料金を払わず、寄付もしていません。
    {/if}
            </td>
          </tr>
        </table>
        <table class="table table-bordered table-hover table-striped">
          <thead>
            <tr><th colspan="2">{$GLOBAL.config.currency} Account Balance/アカウント残高</th></tr>
          </thead>
          <tbody>
            <tr>
